...
module vga_game_display(
    ...
    function in_box;
        input [9:0] px; input [8:0] py;
        input [9:0] x0; input [9:0] x1;
        input [8:0] y0; input [8:0] y1;
        begin
    ...
    always @(*) begin
        if (read_en_n) begin
            ...
            // 修复：在此处强制赋初值，防止生成 Latch 导致黑屏
            rgb_real = 12'h111; 

            // 上方计分区背景
            if (in_box(x, y, 10'd40, 10'd210, 9'd15, 9'd115))  rgb_real = 12'h112;
            if (in_box(x, y, 10'd270, 10'd380, 9'd15, 9'd115)) rgb_real = 12'h121;
            if (in_box(x, y, 10'd460, 10'd570, 9'd15, 9'd115)) rgb_real = 12'h211;

            // 数字显示覆盖
            if (score_digit0 || score_digit1) rgb_real = CYAN;
            if (level_digit)                  rgb_real = GREEN;
            if (error_digit)                  rgb_real = ORANGE;

            // 命中判定区
            if (hit_zone) rgb_real = 12'h333;

            // 轨道边框
            if (lane0_border || lane1_border || lane2_border || lane3_border) rgb_real = GRAY;

            // 轨道内部
            if (lane0) rgb_real = target0 ? RED : DGRAY;
            if (lane1) rgb_real = target1 ? GREEN : DGRAY;
            if (lane2) rgb_real = target2 ? BLUE : DGRAY;
            if (lane3) rgb_real = target3 ? YELLOW : DGRAY;
